...
, mScale(glm::vec3(1.0f)), mPosition(glm::vec3(1.0f)), mRotation(glm::vec3(0.0f))
{
...
glm::mat4x4 Transform::GetModelToWorld() const
{
	...
	glm::vec3 euler = mRotation * glm::pi<float>() / 180.f;
	transform = transform * glm::eulerAngleYXZ(euler.y, euler.x, euler.z);
