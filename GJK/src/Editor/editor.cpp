...
/**
* Initialize the editor that we going to use
*/
...
/**
* Update the editor to draw the windows
*/
...
void cs350::editor::gjk()
{
    ObjectManager* obj_mng = ObjectManager::get_instance();
    GameObject* demo_object1 = obj_mng->FindGameObject("gjk1");
    GameObject* demo_object2 = obj_mng->FindGameObject("gjk2");

    if (demo_object1 != nullptr && demo_object2 != nullptr)
    {
        cs350::renderer& ren = cs350::renderer::instance();
        cs350::window& win = ren.window();

        ImGui::DragFloat3("Position A", &demo_object1->mTransform.mPosition[0], 0.01f);
        ImGui::DragFloat3("Rotation A", &demo_object1->mTransform.mRotation[0], 0.1f);
        ImGui::DragFloat3("Scale A"   , &demo_object1->mTransform.mScale[0]   , 0.01f);

        ImGui::DragFloat3("Position B", &demo_object2->mTransform.mPosition[0], 0.01f);
        ImGui::DragFloat3("Rotation B", &demo_object2->mTransform.mRotation[0], 0.1f);
        ImGui::DragFloat3("Scale B"   , &demo_object2->mTransform.mScale[0]   , 0.01f);

        ImGui::Separator();

        bool simplex = ren.get_simplex_debug_draw();
        if (ImGui::Checkbox("Draw simplex", &simplex))
        {
            ren.set_simplex_debug_draw(simplex);
        }

        int steps = ren.get_steps();
        if (ImGui::DragInt("Steps", &steps, 1, 0, ren.mSimplexs.size()-1))
        {
            ren.set_steps(steps);
        }

        ImGui::Separator();
        bool minkowsi = demo_object2->mMinkowski.get_debug_draw();
        if (ImGui::Checkbox("Minkowsi", &minkowsi))
        {
            demo_object2->mMinkowski.set_debug_draw(minkowsi);
        }
    }
}

/**
* Editor to show the BV
*/
void cs350::editor::bounding_volumes()
{
    ...
    if (demo_object != nullptr)
    {
        ...
        if (ImGui::CollapsingHeader("BVH Types"))
        {
            ...
            ImGui::Checkbox("Draw triangles faces", &demo_object->mBVH.mDrawBVHTrianglesFaces);
...
/**
* Recursive we use to do a tree of the nodes
* @param obj: node that we will put in the tree
