...
namespace cs350 {
    ...
    */
    camera::camera(const glm::vec3 position, const  glm::vec3 up, const float yaw, const  float pitch)
        ...
        mFront(glm::vec3(0.0f, 0.0f, -1.0f)), mMovementSpeed(0.05f), mMouseSensitivity(0.1f),
        mZoom(45.0f), mTarget(0.0f)
    {
        updateCameraVectors();
    }
    /**
    ...
    */
    camera::camera(const float posX, const  float posY, const  float posZ, const  float upX, const  float upY, const  float upZ, const  float yaw, const  float pitch)
        ...
        mFront(glm::vec3(0.0f, 0.0f, -1.0f)), mMovementSpeed(0.05f), mMouseSensitivity(0.1f), mZoom(45.0f),
        mTarget(0.0f)
    {
        updateCameraVectors();
    }
