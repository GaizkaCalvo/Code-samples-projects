...
namespace cs350 {
    ...
    /**
    * @brief
    * check collision with all the objects in the given node
    * @param node
    */
    void demo_octree::check_collisions_octree(const octree<physics_object>::node* node)
    {
        //we finished
        if (node == nullptr)
            return;

        std::vector<const octree<physics_object>::node*> childs;
        //we get the childs of the node
        ...
        if (node->mFirst != nullptr)
        {
            ...
            while (obj1)
            {
                ...
                //we check intersections with all the childs
                for (auto cld : childs)
                {
    ...
    /**
    * @brief
    * get all childs of the given node and store them into the vector
    * @param node
    * @param childs
    */
    void demo_octree::get_all_childs(const octree<physics_object>::node* node, std::vector<const octree<physics_object>::node*>& childs)
    {
        //we get the childs
        for (int i = 0; i < 8; i++)
        {
            if (((node->mChildrenActive >> i) & 1) == 1)
            {
                ...
                const octree<physics_object>::node* child = m_octree_dynamic.find_node(new_locational_code);
                if (child)
                {
    ...
    bool demo_octree::update()
    {
        ...
        for (auto* obj : m_dynamic_objects)
        {
            ...
            {   // Octree update
                ...
                {
                    const octree<physics_object>::node* actual_node = m_octree_dynamic.find_node(obj->bv_world);
